...
{
    char body[HTTP_QUERY_SIZE_MAX]={0,};
    char *getpage = page;
    int len;
    char tmp[10];
    char tpl[1024];
...
int test=0;
        for (i = 0; i < MAX_NUM_USB_DEVICE; i++){
            ...
            if (HTTPHandle->USBStatus[i] == 0){
#endif
//printf("%f  ", HTTPHandle->rcvdRadarData[i][gAppInfo[2].param[2].paramObjNum_V + PROTOCOL_INFO_MAX_SZ]);
test += (int)(HTTPHandle->rcvdRadarData[i][gAppInfo[2].param[2].paramObjNum_V + PROTOCOL_INFO_MAX_SZ] + 0.5);  
                appNum[i] = HTTPHandle->appNum[i];
                
                // idxApp = GetMetaInfo(HTTPHandle->serialNum[i], HTTPHandle->rcvdRadarData[i], appNum[i]);
                idxApp = HTTPHandle->idxApp[i];
                ...
                if (HTTPHandle->rcvDataSize[i] > 0 && HTTPHandle->cntNodata[i] == 0 && idxApp != -1){
                    ...
                    {
                        XK_ClientAddBody_g(TEMPLETE, HTTPHandle->serialNum[i]);
                    }
                    LOG_D("Debug", "%d       %d", HTTPHandle->rcvDataSize[i], HTTPHandle->cntNodata[i]);

                    XK_ClientAddBody_s(TEMPLETE, "serial", HTTPHandle->serialNum[i]);
                    ...
                    if(gTypeData=='m'){
                        {
                            ...
                            for (j = 0; j < gAppInfo[idxApp].appParamSZ; j++){
                                XK_ClientAddBody_f(
                                    ...
                                    HTTPHandle->rcvdRadarData[i][gAppInfo[idxApp].param[j].paramObjNum_V + PROTOCOL_INFO_MAX_SZ]);
                            }
                        }
                    }
                    else{
