...
void *GetRadarDataProcess(void *data)
{
    ...
    while(1)
    {
        ...
        // for (i = 0; i < MAX_NUM_USB_DEVICE; i++){
        //     for (j = 0; j < LED_LIGHT_NUM; j++){
        //         if( strcmp(LED_SerialNumber[j], XK_HTTPHandle->serialNum[i] == 0) ){
        //             if(g_Radar_Data[nShearBufferIdx[i]].m_nALLRadarData[i][PARAM_IDX_ZONE_NUM_PEOPLE]>0 && LED_Status[j]==0){
