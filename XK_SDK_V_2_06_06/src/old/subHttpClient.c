    ...
    for (i = 0; i < RADAR_APP_MAX_SZ; i++)
    {
        if(NumofApp[i]>0 && i == RADAR_APP_WMFALL)
        {
            ...
            for(int noa=0;noa<NumofApp[i];noa++){
                ...
                sprintf(&msgData[strlen(msgData)], bracket_fall, (int)getRadarInfo_fall(GET_INFO_F_SERIAL1,SelR,noa), (int)getRadarInfo_fall(GET_INFO_F_SERIAL2,SelR,noa), (int)getRadarInfo_fall(GET_INFO_F_SERIAL1,SelR,noa), (int)getRadarInfo_fall(GET_INFO_F_SERIAL2,SelR,noa));
