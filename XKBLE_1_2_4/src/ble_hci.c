...
int GetHciDevice(){
	...
	while ( device < 0 ) {
		if(try >= TIMEOUT_CNT) {
			LOG_E("Getting HCI device","Failed to open HCI device. Now Retry to get HCI device [%d/%d]", try, TIMEOUT_CNT);
			exit(0);
		}
		LOG_W("Getting HCI device","Failed to open HCI device. Now Retry to get HCI device [%d/%d]", try, TIMEOUT_CNT);
		delay(1000);
