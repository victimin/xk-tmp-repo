...
void *XK_WatchThread(void *data){
    ...
    while(1){
        ...
        if(gUtcTime.tm_mday != preDay){
            printf("removing old logs...\n");
            system("sudo find /var/log/xk/xkble -ctime +30 -type f -exec rm {} \\;");
        }
        preDay = gUtcTime.tm_mday; 
...
void *ShareRadarDataThread(void *data)
{
    ...
    int cntData_pre[MAX_DEVICE_NUM];
    int preMin=gUtcTime.tm_min;
    ...
    while(1)
    {
        ...
        // for (i = 0; i < configInfo.radarDeviceNum; i++)
        //     printf("%d ", cntData_pre[i] );
        // printf("  \n", gUtcTime.tm_min);
        // for (i = 0; i < configInfo.radarDeviceNum; i++)
        //     printf("%d ", iBconDate[i].cntData );
        // printf("  \n", gUtcTime.tm_min);
        // for (i = 0; i < configInfo.radarDeviceNum; i++)
        //     printf("%d ", iBconDate[i].msgRate );
        // printf("  %d\n\n", gUtcTime.tm_min);
        // for (i = 0; i < configInfo.radarDeviceNum; i++)
        //     printf("%0.2f ", (float)iBconDate[i].msgRate / (float)120 );
        // printf("  %d\n\n", gUtcTime.tm_min);

        if(gUtcTime.tm_min != preMin){
            for (i = 0; i < configInfo.radarDeviceNum; i++){
                iBconDate[i].msgRate = iBconDate[i].cntData - cntData_pre[i];
                cntData_pre[i] = iBconDate[i].cntData;
            }
            preMin = gUtcTime.tm_min;
        }
        ...
        for (i = 0; i < configInfo.radarDeviceNum; i++)
        {
            ...
            if(iBconDate[i].connectionStatus <= CONN_STATUS_NONE){
                ...
                shmDataBuffer_S2M[dataIdx++] = (float)iBconDate[i].msgRate / (float)120;                            ////////////////msg rate
            }
        }
