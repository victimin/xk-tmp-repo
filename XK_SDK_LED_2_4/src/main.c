...
const int GPIO_UART_RST = 18; // Active-low button - Broadcom pin 17, P1 pin 11
...
************************* Do delete ************************/
    ...
    if(argc > 1){
        if(strcmp(argv[1],"-v")==0){
            ...
            for(idx_test=0; idx_test<10; idx_test++){
    ...
    XK_LED_LIGHT_ONOFF(LEDLIGHT_G1_YELLOW_PIN, LED_OFF);
    XK_LED_LIGHT_ONOFF(LEDLIGHT_G1_GREEN_PIN, LED_OFF);
    XK_LED_LIGHT_ONOFF(LEDLIGHT_N0_PIN, LED_OFF);
    XK_LED_LIGHT_ONOFF(LEDLIGHT_N1_PIN, LED_OFF);
    */
    XK_LED_LIGHT_ONOFF(COLOR_IDX_BUZZER, LED_OFF);

    int i,j;
    int k;
    for(i=0;i<MAX_GROUP_NUM;i++){
        if(gLEDInfo.group[i].usingSerial[0][0]>='0' && gLEDInfo.group[i].usingSerial[0][0]<='9'){
            gLEDInfo.numOfGroup++;
        }
        for(j=0;j<MAX_DEV_NUM;j++)
            if(gLEDInfo.group[i].usingSerial[j][0]>='0' && gLEDInfo.group[i].usingSerial[j][0]<='9') gLEDInfo.group[i].numOfSensor++;
    }
...
void SetLEDStatus(int groupN, int numOfPeople){
    if(numOfPeople<gLEDInfo.group[groupN].maxCap){
        ...
        #if (BUZZER_MODE == 1)
            LED_Status[groupN][COLOR_IDX_BUZZER]=LED_OFF;
        #else
    ...
    // }
        ...
        #if (BUZZER_MODE == 1)
            LED_Status[groupN][COLOR_IDX_BUZZER]=LED_ON;
        #else
            LED_Status[groupN][COLOR_IDX_YELLOW]=LED_OFF;
        #endif
...
int GetGroupNum(char *serial){
    int i, j;
    // printf("@@@@@@@@@@@ %d     %d \n",gLEDInfo.numOfGroup, gLEDInfo.group[0].numOfSensor);
    for (i = 0; i < gLEDInfo.numOfGroup; i++){
        for (j = 0; j < gLEDInfo.group[i].numOfSensor; j++){
            //printf("%s     %s \n",gLEDInfo.group[i].usingSerial[j], serial);
...
void *XK_LEDThread(void *data){
    ...
    while(1){  
        ...
        for (i = 0; i < MAX_NUM_USB_DEVICE; i++){
            if(usbStatus[i]==0){
                ...
                if(res == -1 && XK_UARTHandle[i].cntNodata<5){
                    ...
                    else if(combInfo[i].appNum == 31){
                        tmpNumOfPeople[res] = (int)g_Radar_Data[(GETDATA_BUFFER_NUMBER + nShearBufferIdx[i] - 1)%GETDATA_BUFFER_NUMBER].m_nALLRadarData[i][PARAM_IDX_ZONE_COMBO_FINAL_CNT+4];
                        break;
                    }
                }
            }
        ...
        printf("\nHow many people?");
        for (i = 0; i < gLEDInfo.numOfGroup; i++)
            printf("\tG%d=%d\t", i, tmpNumOfPeople[i]);
        printf("\n");
        ...
        for (i = 0; i < gLEDInfo.numOfGroup; i++){
         //printf("######  %d          %d    %d    %d\n", i, LED_Status[i][COLOR_IDX_RED], LED_Status[i][COLOR_IDX_YELLOW], LED_Status[i][COLOR_IDX_GREEN]);
            ...
            #if (BUZZER_MODE == 1)
                XK_LED_LIGHT_ONOFF(gLEDInfo.group[i].ledBuzzerPin, LED_Status[i][COLOR_IDX_BUZZER]);
            #endif
        }
        delay(500);
    }
                    ...
                    switch(XK_UARTHandle[i].appNum){
                        ...
                        case  31:       //Zone-combo
                            ...
                            if(stateCMD[i] == SENDING_STATE_ZONE_VER){
                                ...
                                tmpAtPacket[strlen(tmpAtPacket)] = (((unsigned int)rcvdData[i][PARAM_IDX_ZONE_COMBO_FINAL_CNT+4])/100)%10+'0';
                                tmpAtPacket[strlen(tmpAtPacket)] = (((unsigned int)rcvdData[i][PARAM_IDX_ZONE_COMBO_FINAL_CNT+4])/10)%10+'0';
                                tmpAtPacket[strlen(tmpAtPacket)] = (((unsigned int)rcvdData[i][PARAM_IDX_ZONE_COMBO_FINAL_CNT+4])%10)+'0';

                                tmpAtPacket[strlen(tmpAtPacket)] = (((unsigned int)rcvdData[i][PARAM_IDX_ZONE_COMBO_NUM_ZONE_SENSOR+4])%10)+'0';
                                
                                tmpAtPacket[strlen(tmpAtPacket)] = (((unsigned int)rcvdData[i][PARAM_IDX_ZONE_COMBO_NUM_INOUT_SENSOR+4])%10)+'0';
                        ...
                        case  41:       //Presence-vital
                            ...
                            if(stateCMD[i] == SENDING_STATE_PREVITAL_VER){
                                ...
                                tmpAtPacket[strlen(tmpAtPacket)] = (((unsigned int)rcvdData[i][PARAM_IDX_PREVITAL_VER+4])/1000)%10+'0';
                                tmpAtPacket[strlen(tmpAtPacket)] = (((unsigned int)rcvdData[i][PARAM_IDX_PREVITAL_VER+4])/100)%10+'0';
                                tmpAtPacket[strlen(tmpAtPacket)] = (((unsigned int)rcvdData[i][PARAM_IDX_PREVITAL_VER+4])/10)%10+'0';
                                tmpAtPacket[strlen(tmpAtPacket)] = (((unsigned int)rcvdData[i][PARAM_IDX_PREVITAL_VER+4])%10)+'0';
                            }
                            else{
                                ///////////////// presence normal /////////////////
                                ...
                                tmpAtPacket[strlen(tmpAtPacket)] = (((unsigned int)rcvdData[i][PARAM_IDX_PREVITAL_BR+4])%10)+'0';

                                tmpAtPacket[strlen(tmpAtPacket)] = (((unsigned int)rcvdData[i][PARAM_IDX_PREVITAL_HR+4])/100)%10+'0';
                                tmpAtPacket[strlen(tmpAtPacket)] = (((unsigned int)rcvdData[i][PARAM_IDX_PREVITAL_HR+4])/10)%10+'0';
                                tmpAtPacket[strlen(tmpAtPacket)] = (((unsigned int)rcvdData[i][PARAM_IDX_PREVITAL_HR+4])%10)+'0';

                                tmpMovement = rcvdData[i][PARAM_IDX_PREVITAL_MOVEMT+4] * 10;
                                ...
                                tmpStabltBR = (unsigned int)rcvdData[i][PARAM_IDX_PREVITAL_STBLLT_BR+4];
                                ...
                                tmpStabltHR = (unsigned int)rcvdData[i][PARAM_IDX_PREVITAL_STBLLT_HR+4];
...
void *MqttSendThread(void *data)
{
    ...
    char tmpAtPacket_asc[MQTT_MSG_SZ*2];
    ...
    while(1)
    {
        ...
        if( timeDiff > (long long)XK_HTTPHandle->MqttInfo.frequency / 100 )
        {
            ...
            //memset(tmpAtPacket_asc, 0, MQTT_MSG_SZ*2);
...
#endif
        if(XK_InitInfo.behr_myts_onoff==1){
            threadCreate[threadNum++] = pthread_create(&Mythings_Thread_t, NULL, (void *)SendMythings, (void *)'a');
