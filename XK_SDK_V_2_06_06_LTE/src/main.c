...
void *GetRadarDataProcess(void *data)
{
    ...
    while(1)
    {
        ...
        if(debugDispUSBnum!=255){
            ...
            XK_HTTPHandle->rcvDataSize[portNum] = XK_UARTHandle[portNum].rcvDataSize;
...
//     }
    ...
    if(debugDispUSBnum!=255){
        ...
        sprintf(device, "/dev/ttyACM%d", debugDispUSBnum);
        if( access( device, R_OK | W_OK ) == 0 ){
    ...
    while( -1 !=( param_opt = getopt( argc, argv, "dwsrRtulafv")))
    {
        switch( param_opt)
        {
            case  'd'   :   
                ...
                LOG_I("option","Debug mode - /dev/ttyACM%d is selected!!", debugDispUSBnum);
            break;
...
int XK_USB_Reset(int port){
        ...
        usleep(1000);
        LOG_I("XK_USB_Reset","/dev/ttyACM%d reset successful", port);
...
int XK_USB_Power_Reset(int port){
        ...
        LOG_I("XK_USB_Power_Reset","/dev/ttyACM%d reset successful", port);
